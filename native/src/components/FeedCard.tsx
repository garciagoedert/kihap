...
export default function FeedCard({ post }: FeedCardProps) {
  ...
  const isHtml = post.isHtml || /<[a-z][\s\S]*>/i.test(post.content || '');
  ...
  const cleanContent = finalContent
    ...
    .replace(/margin:[^;]+;/g, '')
    .replace(/padding:[^;]+;/g, '')
    // Remove empty paragraphs generated by rich text editors on double enter (<p><br></p>, <p>&nbsp;</p>, etc)
    .replace(/<p[^>]*>(?:\s|&nbsp;|<br\s*\/?>)*<\/p>/gi, '')
    // Remove redundant trailing or leading <br> inside paragraphs
    .replace(/<br\s*\/?>\s*<\/p>/gi, '</p>')
    .replace(/<p[^>]*>\s*<br\s*\/?>/gi, '<p>')
    // Collapse consecutive <br>
    .replace(/(?:<br\s*\/?>\s*){2,}/gi, '<br>')
    // Remove extra newlines between HTML tags
    .replace(/>\s*\n+\s*</g, '><')
    .trim();
        ...
        <View className="px-4 py-2">
          {!isContentEmpty(cleanContent) && (
            isHtml ? (
              ...
              <Text className="text-gray-800 dark:text-gray-200 text-[14px]" style={{ lineHeight: 22 }}>
