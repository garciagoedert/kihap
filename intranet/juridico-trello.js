...
function renderCards() {
    ...
    boardColumns.forEach(col => {
        const colHtml = `
            <div class="bg-white/50 dark:bg-[#1a1a1a]/50 backdrop-blur-xl rounded-2xl flex flex-col border border-gray-100 dark:border-gray-800/50 kanban-column shrink-0 w-[280px] xs:w-[320px] md:flex-1 md:min-w-[300px] max-w-[400px] max-h-full min-h-0 transition-all duration-300" data-column="${col.id}">
                <div class="flex justify-between items-center p-4 border-b border-gray-100 dark:border-gray-800/50 flex-shrink-0 group/colheader w-full">
                    <!-- View State -->
                    <div class="flex items-center justify-between w-full col-header-view-state" data-column-id="${col.id}">
                        <h2 class="font-bold text-gray-900 dark:text-white uppercase tracking-widest text-[11px] flex items-center gap-2 min-w-0">
                            <div class="w-2 h-2 rounded-full shrink-0" style="background-color: ${getColorHex(col.color)}"></div>
                            <span class="truncate col-title-display-text" title="${col.title}">${col.title}</span>
                            <span class="bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 text-[10px] font-bold px-2 py-0.5 rounded-lg border border-gray-100 dark:border-gray-700 count shrink-0" id="count-${col.id}">0</span>
                        </h2>
                        <button type="button" class="edit-col-title-btn text-gray-400 hover:text-blue-500 transition-colors p-1 opacity-0 group-hover/colheader:opacity-100 focus:opacity-100 shrink-0" data-column-id="${col.id}" title="Editar Coluna">
                            <i class="fas fa-pen text-[10px]"></i>
                        </button>
                    </div>

                    <!-- Edit State (Hidden initially) -->
                    <div class="col-header-edit-state hidden flex flex-col gap-2 w-full">
                        <div class="flex items-center gap-1">
                            <input type="text" value="${col.title}" class="flex-1 bg-white dark:bg-gray-855 border border-gray-300 dark:border-gray-750 rounded px-2 py-0.5 text-xs font-semibold text-gray-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-blue-500 col-edit-title-input" data-column-id="${col.id}">
                        ...
                        <div class="flex justify-between items-center gap-2">
                            <div class="flex items-center gap-1">
                                ...
                                <select class="bg-white dark:bg-gray-850 border border-gray-300 dark:border-gray-750 rounded px-1 py-0.5 text-[10px] text-gray-900 dark:text-white col-edit-color-select focus:outline-none" data-column-id="${col.id}">
    ...
    if (currentDeptId) {
        const addColHtml = `
            <div class="bg-white/30 dark:bg-[#1a1a1a]/30 backdrop-blur-xl rounded-2xl flex flex-col border-2 border-dashed border-gray-300 dark:border-gray-800 shrink-0 w-[280px] xs:w-[320px] md:flex-1 md:min-w-[300px] max-w-[400px] h-[120px] transition-all duration-300 hover:bg-white/45 dark:hover:bg-[#1a1a1a]/45 hover:border-blue-500 dark:hover:border-blue-700 justify-center p-4">
                ...
                <div id="add-board-column-form" class="hidden flex flex-col gap-2.5 w-full">
                    <input type="text" id="new-column-title" placeholder="Nome da coluna..." class="bg-white dark:bg-gray-850 border border-gray-300 dark:border-gray-750 rounded p-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 w-full font-medium">
                    <div class="flex justify-between items-center gap-2">
                        <div class="flex items-center gap-1">
                            <span class="text-[10px] text-gray-500 uppercase tracking-wider font-bold">Cor:</span>
                            <select id="new-column-color" class="bg-white dark:bg-gray-850 border border-gray-300 dark:border-gray-750 rounded px-1.5 py-0.5 text-xs text-gray-900 dark:text-white">
    ...
    document.querySelectorAll('.edit-col-title-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const wrapper = e.currentTarget.closest('.p-4');
            wrapper.querySelector('.col-header-view-state').classList.add('hidden');
            wrapper.querySelector('.col-header-edit-state').classList.remove('hidden');
            wrapper.querySelector('.col-edit-title-input').focus();
        });
    });
    ...
    document.querySelectorAll('.cancel-col-edit-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const wrapper = e.currentTarget.closest('.p-4');
            wrapper.querySelector('.col-header-view-state').classList.remove('hidden');
            wrapper.querySelector('.col-header-edit-state').classList.add('hidden');
        });
    });
    ...
    document.querySelectorAll('.save-col-edit-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            ...
            const wrapper = e.currentTarget.closest('.p-4');
            const newTitle = wrapper.querySelector('.col-edit-title-input').value.trim();
            const newColor = wrapper.querySelector('.col-edit-color-select').value;
            if (!newTitle) {
...
formCreateDept.addEventListener('submit', async (e) => {
    ...
    try {
        await addDoc(deptsCol, { name, color, columns: DEFAULT_COLUMNS, createdAt: serverTimestamp() });
